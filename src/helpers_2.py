...
def group_date(df, columns):
    ...
    # compute dataframe containing the average of each columns over a month
    ...
    # compute dataframe containing the standard deviation of each columns over a month
    ...
    # concatenate the dataframes with respect to the date
...
def linear_interpolation(df, columns):
    ...
    for i, date in enumerate(date_list):
        ...
        if date not in df.index and i != 0:
            ...
            # Search for the next date that is in dataframe
            new_index = i
            while (
