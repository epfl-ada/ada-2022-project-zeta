...
parties = {
    ...
    "dk": {
        "Socialdemokraterne": "SD *",  # center-left
        "Venstre": "Venstre",       # center-right  
        "Liberal Alliance": "LA",  # center-right
        "Socialistisk Folkeparti": "SF", # radical left
        "Det Konservative Folkeparti": "Konservative",  # center-right

    },
    "es": {
        "Partido Popular": "PP", # center-right
        "Partido Socialista Obrero Español": "PSOE *", # center-left
        "Unidos Podemos": "UP", # left-wing
        "Ciudadanos–Partido de la Ciudadanía": "Cs", # centrism
        "Vox": "Vox", 
    },
    "fi": {
...
def linear_interpolation(df, columns):
    ...
    # list of months
    date_list = ['2019-01', '2019-02', '2019-03', '2019-04', '2019-05', '2019-06', '2019-07', '2019-08', '2019-09', '2019-10', '2019-11', '2019-12',
    ...
    for i, date in enumerate(date_list):
        if date not in df.index and i != 0:
            ...
            # save the last date that is in the dataframe
            before_date = date_list[i-1]
            
            # search for the next date that is in the dataframe
            new_index = i
            while date_list[new_index] not in df.index and new_index < len(date_list)-1:
                new_index += 1
            
            new_date = date_list[new_index]

            # number of unvalid dates between valide ones
            ...
            while i != new_index:
                ...
                curr_t = intervals[no_interval]

                for column in columns:
                    df.loc[curr_date, "avg_" + column] = (1 - curr_t) * df.loc[before_date, "avg_" + column] + curr_t * df.loc[new_date, "avg_" + column]
                    df.loc[curr_date, "std_" + column] = (1 - curr_t) * df.loc[before_date, "std_" + column] + curr_t * df.loc[new_date, "std_" + column]
